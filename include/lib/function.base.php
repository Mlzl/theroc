...
function breakLog($content, $lid) {
    ...
    if (!empty($ret[1])) {
        $ret[0].='<p class="readmore"><a href="' . Url::log($lid) . '">read more&gt;&gt;</a></p>';
        return $ret[0];
    } elseif(Option::get('isexcerpt') == 'y') {
        return subString(trim(strip_tags($content)), 0, Option::get('excerpt_subnum')) . '<p class="readmore"><a href="' . Url::log($lid) . '"read more&gt;&gt;</a></p>';
    } else {
        return $content;
    }
}
