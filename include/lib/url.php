...
class Url {
    ...
    static function log($blogId) {
        ...
        switch ($urlMode) {
            case '0'://默认：动态
                $logUrl = BASE_URL . '?post=' . $blogId;
                break;
            case '1'://静态
                $logUrl = BASE_URL . 'post-' . $blogId . '.html';
                break;
            case '2'://目录
                $logUrl = BASE_URL . 'post/' . $blogId;
                break;
            case '3'://分类
                ...
                if (!empty($log_sort[$blogId]['alias'])) {
                    $logUrl = BASE_URL . $log_sort[$blogId]['alias'] . '/' . $blogId;
                } elseif(!empty($log_sort[$blogId]['name'])) {
                    $logUrl = BASE_URL . $log_sort[$blogId]['name'] . '/' . $blogId;
                } else {
                    $logUrl = BASE_URL . $blogId;
                }
                $logUrl .= '.html';
                break;
        }
