...
class LoginAuth{
    ...
    public static function isLogin() {
        ...
        $roc_cookie_key = 'roc_key';
        $uid_mk5_cookie_key = 'roc_u';
        if(empty($_COOKIE['roc_key']) || empty($_COOKIE['roc_u'])){
            return false;
        }
        $roc_key = $_COOKIE[$roc_cookie_key];
        $roc_u = $_COOKIE[$uid_mk5_cookie_key];  # 暂时不验证
        $userData = self::getUserInfoFromRedis($roc_key);
        if(!$userData){
            return false;
        }
        $GLOBALS['user_name'] = $userData['user_name'];
        $GLOBALS['user_id'] = $userData['user_id'];
    ...
    public static function getUserInfoFromRedis($roc_key){
        $redis = self::getRedis();
        $key = 'theroc:login:user:info:'.$roc_key;
    ...
    public static function getRedis(){
        ...
        if(!$redis){
            $redis = new Redis();
            $redis->connect(getenv('THEROC_REDIS_HOST'), getenv('THEROC_REDIS_PORT'));
        }
        return $redis;
    }
